...
Each component can also run locally (`python -m intake.main`, `python -m search.main`, `python -m report.main`). By default (`APP_ENV=local`) each process wires its *own* in-memory broker and store, so local runs exercise one component at a time rather than the full pipeline — the components only talk to each other through real Pub/Sub when deployed. Setting `APP_ENV=gcp` (with the environment variables in [shared/config.py](shared/config.py)) switches a process to the production Cloud SQL, Pub/Sub, Redis, Exa, and Claude adapters.
...
```mermaid
flowchart LR
    User([Examiner<br/>PDF + claims + critical date])
    FE["Frontend<br/>localhost:3000"]

    subgraph VMA ["VM A — intake/"]
        A["Component A<br/>FastAPI intake"]
    end
    ClaudeA["Claude:<br/>claim analysis"]
    subgraph VMB ["VM B — search/"]
        B["Component B<br/>iterative search agent"]
    end
    ClaudeB["Claude:<br/>search decisions"]
    ExaSearch["Exa Search API"]
    subgraph VMC ["VM C — report/"]
        C["Component C<br/>report worker"]
    end
    ClaudeC["Claude:<br/>evidence ranking"]
    ExaContents["Exa Contents API"]

    PS1[/"Pub/Sub<br/>search-plans"/]
    PS2[/"Pub/Sub<br/>candidates"/]
    Redis[("Memorystore Redis<br/>query cache")]
    SQL[("Cloud SQL<br/>jobs + reports")]

    User -->|"submit, then poll by job ID"| FE -->|"proxies /jobs"| A

    A -->|"claims text"| ClaudeA
    ClaudeA -->|"claim map + initial queries"| A
    A -->|"create job / mark failed"| SQL
    SQL -->|"status + report on polls"| A
    A -->|"search plan"| PS1 --> B

    B -->|"uncached queries"| ExaSearch
    ExaSearch -->|"result metadata"| B
    B <-->|"check before Exa,<br/>store results (24 h TTL)"| Redis
    B -->|"limitations, tried queries,<br/>candidate snippets"| ClaudeB
    ClaudeB -->|"finish, or 3–6<br/>follow-up queries"| B
    B -->|"candidates + effective plan"| PS2 --> C

    C -->|"candidate URLs"| ExaContents
    ExaContents -->|"full page text"| C
    C -->|"candidates + content"| ClaudeC
    ClaudeC -->|"ranked evidence + passages"| C
    C -->|"report + completed status<br/>(one transaction)"| SQL
```
...
3. **Search.** Component B pulls the plan and runs a bounded agentic loop (detail below): execute queries through Exa (checking Redis first), filter by the critical date, deduplicate, then ask Claude whether coverage is sufficient or 3–6 targeted follow-up queries are worth running. When the loop ends, B publishes up to 25 candidates plus the *effective plan* — the original queries and every follow-up actually executed — so provenance survives.
4. **Rank.** Component C pulls the batch, fetches full page content for every candidate through Exa (falling back to snippet-only with an uncertainty note if a URL keeps failing), and asks Claude to rank the evidence against specific claim limitations with supporting passages. Python re-validates everything: ranked URLs must come from the batch, citation URLs must match their candidate, and Claude cannot alter source metadata — it returns only rank, explanation, passages, and uncertainty notes; each result row is rebuilt from the original candidate.
5. **Retrieve.** C stores the report and the `completed` status in one database transaction. The examiner polls `GET /jobs/{job_id}` on Component A, which reads status and the finished report from the same database.

### Component B's search loop — the agentic part
...
| --- | --- | --- |
| Messaging | GCP Pub/Sub | Two topics carry the work between processes: `search-plans` (A → B) and `candidates` (B → C). Delivery is at-least-once, so every consumer is idempotent, and a message is only acknowledged after the component's own output has been published or stored. |
| Caching | Memorystore Redis | B checks Redis before every Exa query (equivalent queries share an entry after normalization; the critical date is part of the key) and caches results for 24 h — resubmitting a job visibly turns misses into hits. Redis also holds per-job completion keys that suppress duplicate candidate batches. |
| Database | Cloud SQL PostgreSQL | The system of record for jobs and reports. A creates and polls job rows; C writes the report and `completed` status atomically, first-write-wins, so duplicate deliveries can never produce duplicate reports. |
...
- **Untrusted data stays data.** Uploaded patent text, Exa snippets, and retrieved web content are passed to Claude as tagged untrusted content, never as instructions. Document text is kept out of logs, Pub/Sub messages, and Redis keys; API keys and the database DSN are masked in config reprs; Component B never receives database credentials at all.

## Project structure

| Path | What it is |
| --- | --- |
| [intake/](intake/) | **Component A.** FastAPI service: upload validation and PDF text extraction ([extraction.py](intake/extraction.py)), the create-job → analyze → publish pipeline ([pipeline.py](intake/pipeline.py)), the `POST /jobs` / `GET /jobs/{id}` API ([api.py](intake/api.py)), and the production Claude claim-analysis adapter ([claude_adapter.py](intake/claude_adapter.py)). |
| [search/](search/) | **Component B.** The Redis-aware query cache ([cache.py](search/cache.py)), date filtering and dedup ([consolidate.py](search/consolidate.py)), concurrent Exa execution ([executor.py](search/executor.py)), the bounded iterative loop ([loop.py](search/loop.py)), the Claude search-decision adapter ([claude_adapter.py](search/claude_adapter.py)), and the Pub/Sub worker ([worker.py](search/worker.py)). |
| [report/](report/) | **Component C.** Content retrieval and ranking pipeline ([pipeline.py](report/pipeline.py)), the Claude ranking adapter ([claude_adapter.py](report/claude_adapter.py)), and the worker that handles terminal failures, duplicates, and storage ([worker.py](report/worker.py)). |
...
| Tool | Why |
| --- | --- |
| Python 3.12 + FastAPI + uvicorn | Component A's API; B and C are plain background worker processes. |
...
| LangChain (`langchain-anthropic`) + Claude Sonnet 5 | Structured-output calls for the three AI roles: claim analysis (A), search decisions (B), evidence ranking (C). Temperature 0 where the model allows it, bounded retries, finite timeouts. |
| Exa (`exa-py`) | Web search over non-patent literature (B) and full-page content retrieval (C), with published-date metadata. |
...
- The critical date is supplied by the examiner and taken as given (any valid calendar date). Same-day publications count as on-or-before the critical date; results dated *after* it are dropped; **undated results are kept** and flagged `unknown` so Component C and the human can weigh them, rather than silently discarding possible art.
...
- Message contracts have no schema-version field; all three components are always deployed from the same commit.
...
- **Local mode is per-component.** Each local entry point creates an isolated in-memory broker and store, so the full A→B→C pipeline only runs against real Pub/Sub in the GCP deployment.
...
- Content retrieval in C is sequential with no retry backoff (worst case 75 Exa content requests for a 25-candidate batch), and 25 full bodies can make a large ranking prompt; real-world token cost and ranking quality haven't been benchmarked.
